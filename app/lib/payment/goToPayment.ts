...
export async function goToPayment(employee: Employee & { user: User }, date: Date, time: string, service: Service, timeZone: string) {
    const serviceStripe = {
        price_data: {
            currency: "EUR",
    ...
    if (!tempAppointment) {
        redirect(`${process.env.NEXT_PUBLIC_BASE_URL}/home/services?status=failed`);
    }
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL}/home/services?status=cancelled`,
        metadata: {
